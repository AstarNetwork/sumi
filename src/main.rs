...
{{ for function in functions }}
        ...
        #[ink(message)]
        pub fn {function.name}({{ for input in function.inputs }}{input.name}: {input.ink_type}{{ if not @last }}, {{ endif }}{{ endfor }}) -> {function.output} \{
            let encoded_input = Self::{function.name}_encode({{ for input in function.inputs }}{input.name}.into(){{ if not @last }}, {{ endif }}{{ endfor }});
            self.env()
                .extension()
                .xvm_call(
                    super::EVM_ID,
        ...
        fn {function.name}_encode({{ for input in function.inputs }}{input.name}: {input.evm_type}{{ if not @last }}, {{ endif }}{{ endfor }}) -> Vec<u8> \{
...
fn main() -> Result<(), String> {
    ...
    template.add_formatter("upper_snake", |value, buf| match value {
        serde_json::Value::String(s) => Ok(*buf += &s.to_case(Case::UpperSnake)),
